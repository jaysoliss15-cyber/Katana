...
object ProfileStore {
    ...
    fun toBundle(profile: DeviceProfile = getActiveProfile()): Bundle {
        return Bundle().apply {
            ...
            putString(NPatchConfig.KEY_ACTIVE_SYNTHETIC_IP, profile.testIpv4)
            putString("wifi_ssid", profile.wifiSsid)
            putString(NPatchConfig.KEY_ACTIVE_WIFI_SSID, profile.wifiSsid)
            putString("wifi_bssid", profile.bssid)
            putString(NPatchConfig.KEY_ACTIVE_WIFI_BSSID, profile.bssid)
